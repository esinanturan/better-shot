        ...
        GeometryReader { proxy in
            if let sourceImage = model.sourceImage {
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    if model.isCropping {
                        CropBox(rect: $model.cropRect, frameSize: sourceImageFrame.size)
                ...
                .animation(reduceMotion ? nil : .spring(response: 0.36, dampingFraction: 0.88), value: model.imageSize)
                .animation(reduceMotion ? nil : InspectorMotion.reveal, value: model.isCropping)
                .overlay(alignment: .bottom) {
                    if model.isCropping {
                        CropToolbar(
                            dimensions: "\(Int(model.cropDimensions.width)) x \(Int(model.cropDimensions.height))",
                            canReset: !model.isCropRectPristine,
                            onReset: { withAnimation(reduceMotion ? nil : InspectorMotion.reveal) { model.cropRect = CropGeometry.identity } },
